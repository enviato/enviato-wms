...
1. **CUSTOMER read surface.** What should a CUSTOMER user literally see when they log in? Packages addressed to them? Invoices billed to them? AWBs they're the consignee on? Their own `customers_v2` row? This answers F-4. Proposal: (packages where `customer_id = self`) + (invoices where `customer_id = self`) + (AWBs where at least one packages row has `customer_id = self`) + (their own notifications + users row).
   ...
   **STATUS 2026-04-21 — RESOLVED (partial) + DEFERRED (customers_v2):** Packages / invoices / AWBs / package_photos / users closed by `019_customer_read_surface.sql` (Phase 10B). The remaining open slice was `customers_v2`. Call-site audit 2026-04-21 grepped all of `src/` for `customers_v2` references and found **zero**: every "customer" read in the dashboard goes through `public.users` (e.g. `users!packages_customer_id_fkey`, `users!invoices_customer_id_fkey`), and `src/app/api/admin/create-recipient/route.ts` writes CUSTOMER recipients into `public.users` with `role_v2='CUSTOMER'`, not `customers_v2`. `customers_v2` is schema-resident with RLS but has no UI call site today, so no CUSTOMER-read path breaks. **Decision:** defer the CUSTOMER SELECT branch on `customers_v2` until a feature actually needs it — designing the branch now means picking between three mutually-exclusive design options (see Q5) without a concrete consumer to validate the choice. Reopen this question the day any UI starts reading `customers_v2`. No migration written; no regression test added; this resolution is documentation-only.
...
5. **customers_v2 self-access.** Is `customers_v2.id` intended to equal `users.id` for CUSTOMER users? Or is it a separate address record? Affects how F-4 fix is written.
   ...
   **STATUS 2026-04-21 — ANSWERED (introspected) + DEFERRED (decision).** Direct DB introspection: `customers_v2.id` is a **separate UUID**, not `users.id`. Evidence from prod fixtures: 10 CUSTOMER users in `public.users`, 12 rows in `customers_v2`, **0** id-column matches, **10** email matches, **0** orphan CUSTOMER users. `email` is unique in `customers_v2` (no duplicates). Schema also has no `user_id` / `auth_id` FK back to `users`. So the relationship is implicit via `email` only; the two extra `customers_v2` rows look like walk-in customers without user accounts. If/when a CUSTOMER-facing feature needs to read `customers_v2`, the design branch is a three-way choice between: (A) email-based RLS (`EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND email = customers_v2.email AND role_v2='CUSTOMER')`); (B) JWT email claim (fastest, couples RLS to JWT `email` claim); (C) add an explicit `customers_v2.user_id` FK column + backfill + update the create-recipient write path. Deferred jointly with Q1 — will pick (A/B/C) the day a real consumer exists.
